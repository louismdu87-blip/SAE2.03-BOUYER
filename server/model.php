...
{
    $cnx = new PDO("mysql:host=" . HOST . ";dbname=" . DBNAME, DBLOGIN, DBPWD);
    // Correction : Table 'Favoris', Table 'Notes', et colonne 'Notes.note'
    $sql = "SELECT Movie.*, ROUND(AVG(Notes.note), 1) AS moyenne_notes FROM Movie 
            INNER JOIN Favoris ON Movie.id = Favoris.id_movie 
            LEFT JOIN Notes ON Notes.id_movie = Movie.id
...
{
    $cnx = new PDO("mysql:host=" . HOST . ";dbname=" . DBNAME, DBLOGIN, DBPWD);
    $sql = "INSERT IGNORE INTO Favoris (id_profile, id_movie) 
